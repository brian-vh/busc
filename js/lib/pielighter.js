$.fn.pielighter = function(opts) {
	...
	function doMath(event) {
		...
		if (pie.Quadrant == 2) {
			pie.radians = (endAngle+5 - 1.5 * Math.PI);
	...
	function renderHighlight(event) {
		...
		// clear canvas
		var dlina_okr=2*radius*Math.PI;
		//drawContainer();
		var times_item=0,proc_item=85;
		...
		ctx.lineTo(radius, radius);
            ...
            var grad = ctx.createRadialGradient(radius, radius, radius, radius, radius, 0);
            grad.addColorStop(0, '#0066b3');
            grad.addColorStop(0.11, '#0066b3');
            grad.addColorStop(0.12, '#0094d5');
            grad.addColorStop(0.5, '#008ad0');
            grad.addColorStop(0.7, '#0072bb');
            grad.addColorStop(1, '#005cac');
			ctx.fillStyle = grad;
